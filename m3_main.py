...
def write_to_rank(rank, text):
    ...
    with open(f'm3_data/rank{rank}.txt', 'a', encoding='utf-8') as file:
        file.write(text + '\n')
...
def html_formatting(email_body):
    ...
    search_queries = 'This search was performed using the terms "Clostridium difficile/Clostridioides difficile/' \
                     'C. difficile/difficile/CDI" and "recurrent/recurrence/Dale Gerding/Mark Wilcox/Sahil Khanna"'
    ...
    html = f"""
            ...
            <body>
                <img src="cid:image">
                <div class="container">
                    <h1 style="text-align: center;">New high priority papers are available on PubMed (NTCD-M3 {date})</h1>
                </div>
                {email_body}
                <hr> <!-- Black line -->
                <p class="small-text">
...
def send_email(email_text):
    ...
    subject = f'New high priority literature (NTCD-M3 {date})'  # Title of the email
    body = email_text
...
if __name__ == '__main__':
    ...
    with open('m3_data/m3_queries.txt', 'r') as search_queries:
    ...
    for query in queries:
        ...
        for doi, info in bot_search.items():
            status, doi = doi_checker(doi, 'm3_data/doi_db.txt')
    ...
    with open('m3_data/log.txt', 'a') as log:
